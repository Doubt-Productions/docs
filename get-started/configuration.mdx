...
3. While `staffRoles` is still present, run **`/botstaff migrate`** to import support-guild members who hold those roles into `BotStaff`. If you already removed `staffRoles`, add each person with **`/botstaff add <user>`** instead.
