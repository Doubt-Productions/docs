...
1. While `moderation.staffRoles` is still in your live `config.js`, run **`/botstaff migrate`** to import support-guild members who hold those roles into `BotStaff`.
2. If you already removed `staffRoles`, add each person with **`/botstaff add <user>`** before deploy.
