...
`/eval` runs code in an isolated VM sandbox (`safeEval.js`). It is developer-only — the user's Discord ID must appear in the bot's developers allowlist (`config.moderation.developers`). If the allowlist is missing or empty, eval is denied.
...
| `/eval` | Developer allowlist + VM sandbox |
