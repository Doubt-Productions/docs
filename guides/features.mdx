...
3. Run **`/botstaff migrate`** to import members from legacy roles.
4. Confirm with **`/botstaff list`**, then remove `staffRoles` from config.

If you already removed `staffRoles` from config, skip migrate and add each person with **`/botstaff add`** instead (after steps 1–2).
