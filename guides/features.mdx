...
Mention detection uses Discord's `message.mentions` metadata — the bot does not read message text and does not require Message Content Intent.

### Returning from AFK

AFK is automatically cleared when the user sends a message (no message content is read). Alternatively:
...
1. Deploy the BotStaff update while keeping `moderation.staffRoles` in your live `config.js` (do not remove it yet).
...
3. While `staffRoles` is still present, run **`/botstaff migrate`**. If you already removed `staffRoles`, add each person with **`/botstaff add`** instead.
