...
1. Deploy the BotStaff update, but keep `moderation.staffRoles` in your live `src/config.js`.
2. Run `npx prisma db push` so the `botstaff` collection exists (required before any `/botstaff` subcommand).
3. Run `/botstaff migrate` while the legacy role IDs are still present. Migration reads `moderation.staffRoles`, resolves the support guild (`variables.supportServerId` or `handler.guildId`), and creates `BotStaff` rows (with badge sync) for members who hold any listed role.
4. Confirm with `/botstaff list`, then remove `staffRoles` from config.

If you already removed `staffRoles`, skip migrate and add each person manually with `/botstaff add <user>` instead (after steps 1–2).
