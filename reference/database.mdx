...
<Note>
The reserved `bot-staff` badge (🛡️ Bot Staff) is created and assigned automatically by `/botstaff`. It cannot be managed through `/badge` — see [Security](/security#global-bot-staff-acl).
</Note>
...
Global bot staff access-control list — one row per staff member (cross-guild).

| Field | Type | Description |
|-------|------|-------------|
| `id` | ObjectId | Auto-generated primary key |
| `userId` | String | Discord user ID (unique) |
| `addedBy` | String | Discord user ID of the developer who added this staff member |
| `addedAt` | DateTime | When the staff member was added (default: now) |

**Collection:** `botstaff`

**Used by:** `/botstaff` (add/remove/list/migrate), `staffOnly` command gate in `devCommandValidator.js`

**Indexes:** `userId` is unique — one `BotStaff` row per Discord user.

<Warning>
After schema changes that add this model, run `npx prisma db push` before using `/botstaff` or staff-only commands in production.
</Warning>
...
<Note>
AFK uses `messageCreate` for clearing status and mention notifications via `message.mentions`. It does not read `message.content` and does not require Message Content Intent.
</Note>
...
<Note>
Legacy `guildschemas` documents may still exist in MongoDB from older prefix-command builds. They are inert after the `GuildSchema` model was removed and can be ignored or cleaned up manually.
</Note>
